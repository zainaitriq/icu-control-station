...
try {
    ...
    Write-Host "   ⚠️  Installing javascript-obfuscator..." -ForegroundColor Yellow
...
PORT=3001
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\config\.env.template"
...
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\scripts\update.sh"
...
ls -lh `$BACKUP_FILE
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\scripts\backup.sh"
...
free -h | grep Mem
echo ""
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\scripts\health-check.sh"
...
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\INSTALL.md"
...
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\backend-obfuscated\certs\README.txt"
...
© 2025 All Rights Reserved
"@ | Set-Content "$BUILD_DIR\$PACKAGE_NAME\README.md"

# Step 6: Create archive using tar (if available) or zip
Write-Host ""
Write-Host "📦 Step 6: Creating deployment archive..." -ForegroundColor Cyan

Push-Location $BUILD_DIR

# Try to use tar if available (Git Bash or WSL)
try {
...
Write-Host "   2. Extract: tar -xzf $PACKAGE_NAME.tar.gz" -ForegroundColor Gray
Write-Host "   3. Run: cd $PACKAGE_NAME && sudo ./install.sh" -ForegroundColor Gray
Write-Host ""
